...
public interface IProductRepository extends JpaRepository<Product,Long> {
   ...
   List<Product> findAllByVendor(Vendor vendor);
    ...
    List<Product> findAll(String productNumber, String productName, Double minProductPrice, Double maxProductPrice, ProductStatus status);
    @Query("SELECT p FROM Product p WHERE lower(p.productNumber) like %?1% " +
            "and LOWER(p.productName) like %?2% " +
            "and p.status = ?3 order by p.productName")
    List<Product> findByNameNumber(String productNumber, String productName, ProductStatus status);
    ...
    List<Product> findByNamePrice(String productName, Double minProductPrice,
                                  Double maxProductPrice, ProductStatus status);
    ...
    List<Product> findByName(String productName, ProductStatus status);

    @Query("SELECT p FROM Product p WHERE lower(p.productNumber) like %?1% " +
            "and p.status = ?2 order by p.productName")
    List<Product> findByNumber(String productNumber, ProductStatus status);
    ...
    @Query("SELECT p FROM Product p WHERE " +
            ...
            "and p.status = ?3 order by p.productName")
    List<Product> findByPrice(Double minProductPrice,Double maxProductPrice, ProductStatus status);
